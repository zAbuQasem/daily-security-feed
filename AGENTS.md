...
**notify_discord.py** sends one daily feed embed on success (research only), sorted by priority, with a "Browse feed" link to the GitHub Pages site when `PAGES_URL` is set. On failure, it sends a red embed with failed step, run link, and feed link.
